...
% Section S6: solve internal actions model
    ...
    if fSDP
     if fOK, [p_in,T_in] = S3PRE_inlet(p_suc , T_suc , INport_Amax, INport_Amin, V_comp(1), MM_g , n_van,rpm,c,toll_d,c_v,coeff_invalve,pipe,cpitch,ct,lenght,D_up,D_do,roughness,mu_g,coeff_infilter);
     end
    else
        p_in = p_suc;
        T_in = T_suc;
...
%   end
   ...
   % ==================================================================== 
    ...
    if fOK && c == 1
        ...
        save (IO.fullsavename,'FLAG','NUMERIC','PROCESS','GEOMETRY','GAS','LIQUID','LEAK','VANE','SHAFT','BUSHING','NOZZLES','STRESS','STRESSsh','RESULTS','INTAKE');
