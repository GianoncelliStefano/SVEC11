function A = SX_Catstruct(varargin)
% CATSTRUCT   Concatenate or merge structures with different fieldnames
%   X = SX_Catstruct(S1,S2,S3,...) merges the structures S1, S2, S3 ...
...
%     S5.honest = false ;
%     Y = SX_Catstruct(S1,S2,S3,S4,S5) % use value from S4
%
...
%     C(1).bb = 1 ; C(2).bb = 2 ;
%     D(1).aa = 3 ; D(2).aa = 4 ;
%     CD = SX_Catstruct(C,D) % CD is a 1x2 structure array with fields bb and aa
%
%   The last input can be the string 'sorted'. In this case,
%   SX_Catstruct(S1,S2, ..., 'sorted') will sort the fieldnames alphabetically. 
%   To sort the fieldnames of a structure A, you could use
%   SX_Catstruct(A,'sorted') but I recommend ORDERFIELDS for doing that.
%
...
if ~isstruct(varargin{end}),
    if isequal(varargin{end},'sorted'),
        ...
        error('SX_Catstruct:InvalidArgument','Last argument should be a structure, or the string "sorted".') ;
    end
else
    sorted = 0 ;
end
...
for ii=1:N,
    X = varargin{ii} ;
    if ~isstruct(X),
        error('SX_Catstruct:InvalidArgument',['Argument #' num2str(ii) ' is not a structure.']) ;
    end
    ...
    if ~isempty(X),
        ...
        if ii > 1 && ~isempty(sz0)
            if ~isequal(size(X), sz0)
                error('SX_Catstruct:UnequalSizes','All structures should have the same size.') ;
            end
        else
            sz0 = size(X) ;
        end
...
elseif NonEmptyInputsN == 1,
    ...
    if numel(UFN) ~= numel(FN),
        warning('SX_Catstruct:DuplicatesFound','Fieldnames are not unique between structures.') ;
        sorted = 1 ;
    end
