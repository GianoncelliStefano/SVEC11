function varargout = SX_v2struct(varargin)
...
%      S = SX_v2struct
%      S = SX_v2struct(x,y,z,...)
%      S = SX_v2struct(fieldNames)
%      S = SX_v2struct(A,B,C,..., fieldNames)
%      S = SX_v2struct(x,..., nameOfStruct2Update, fieldNames)
%      SX_v2struct
%      SX_v2struct(x,y,z,...)
%      SX_v2struct(fieldNames)
%      SX_v2struct(A,B,C,..., fieldNames)
%      SX_v2struct(x,..., nameOfStruct2Update, fieldNames)
%
%    Unpack
%      SX_v2struct(S)
%      [a,b,c,...] = SX_v2struct(S)
%      SX_v2struct(S,fieldNames)
%      [a,b,c,...] = SX_v2struct(S,fieldNames)
%
%% Inputs & Outputs
%    Pack - inputs
...
%       S = SX_v2struct(fieldNames1)
%      % Example 3.
%      % same as #1. but arguments are passed explicitly
%       S = SX_v2struct(zeros(3), 'Testing123', cell(2,3), fieldNames1)
%      % Example 4.
...
%       S = SX_v2struct(x,y,z, fieldNames2)
%
%      % Example 5.
...
%       S = SX_v2struct(x2, nameOfStruct2Update, fieldNames3)
%
%      % Example 6.
...
%       SX_v2struct(x,y,z)
%      % Example 8.
%       SX_v2struct(fieldNames1)
%      % Example 9.
%       SX_v2struct(zeros(3), 'Testing123', cell(2,3), fieldNames1)
%      % Example 10.
%       SX_v2struct(x,y,z, fieldNames2)
%      % Example 11.
%       S.oldField = 'field to be saved for future use'
%       SX_v2struct(x2, nameOfStruct2Update, fieldNames3)
%      % Example 12.
%       SX_v2struct
%
%  % Unpack
        ...
        if isequal(nInputs, 0)
            ...
            if gotCellArrayOfStrings
                ...
                warnMsg = ['V2STRUCT - ''%s packed in the structure.'...
                    ...
                    ' ''%s''.'];
                fprintf('\n')
                warning('MATLAB:V2STRUCT:cellArrayOfStringNotFieldNames',warnMsg,msgStr,name,name,name)
            end
        end
